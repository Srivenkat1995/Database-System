...
public class Utility
{
    ...
    public static HashMap<String,Integer> getFieldPostionMappingForJoin(String Table)
    {
        HashMap<String,Integer> joinMap = new HashMap<String, Integer>();

        List<Field> fieldList = TableInformation.getTableFieldMappingInfo(Table);

        for(int i=0; i < fieldList.size(); i++)
        {
            String field = fieldList.get(i).getColumnName();
            joinMap.put(Table+"."+field,i);
            joinMap.put(field,i);
        }
        return joinMap;
    }
